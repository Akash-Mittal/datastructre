...
public class Graph<E> {
    ...
    private void addAdjacentNodes(Node<String> node, int bucketLocation) {
        ...
        if (Objects.nonNull(this.nodeArray[bucketLocation])) {
            for (tempNode = (Node<String>) this.nodeArray[bucketLocation]; Objects
                    .nonNull(tempNode.getNext()); tempNode = tempNode.getNext())
                ;
    ...
    public List<E> depthFirstSearch(Node<E> nodeArray[]) throws Exception {
        List<E> traversed = new LinkedList<E>();

        Stack<E> stack = new Stack<E>();
        for (int i = 0; i < nodeArray.length; i++) {
            if (!stack.contains(nodeArray[i].getData()) && (!traversed.contains(nodeArray[i].getData()))) {
                traversed.add(nodeArray[i].getData());
                for (Node<E> tempNode = nodeArray[i].getNext(); Objects
                        .nonNull(tempNode); tempNode = tempNode.getNext()) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        intArray[3][2] = 0;
        intArray[3][3] = 0;
        List<String> path = graph.depthFirstSearch((Node<String>[]) graph.buildLinkedRepresentationFromArray(intArray));
